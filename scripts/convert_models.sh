...
# Downloads ~10.5 GB: GGUF Q4_K DiT (4.2 GB), Qwen-Image-2.1 VAE (1.35 GB), Qwen3-VL-8B MNN (5.1 GB).
set -e
...
hf download taobao-mnn/Qwen3-VL-8B-Instruct-MNN --exclude "visual.mnn*" --local-dir "$SRC/qwen3-vl-8b-mnn"
...
# VAE decoder, fp16 weights, residual stream rescaled for fp16
"$PY" "$ROOT/export/export_vae.py" --vae "$SRC/qwen-image-2.1/vae" --out "$OUT" --fp16
...
for f in llm.mnn llm.mnn.weight embeddings_int4.bin tokenizer.txt llm_config.json; do
